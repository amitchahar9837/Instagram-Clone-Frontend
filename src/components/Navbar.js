...
const Navbar = () => {
    ...
    const searchUser = (query) => {
        setSearch(query)
        fetch(summaryApi.searchUser.url, {
            ...
            <>
                <div className={`${showSearch ? 'inline-block' : 'hidden'} relative z-10`} aria-labelledby="modal-title" role="dialog" aria-modal="true">
                    ...
                    <div className={`fixed inset-0 bg-gray-300 bg-opacity-75 transition-opacity flex flex-col items-center `}>
                        ...
                        <ImCross className='absolute top-5 left-5 text-black hover:text-red-400 text-xl cursor-pointer' onClick={() => setShowSearch((prev) => !prev)} />
        ...
        <>
            <nav className="bg-slate-100 shadow-[0px_1px_8px_2px_rgba(0,0,0,0.3)] sticky top-0 left-0 z-[100]">
                <div className='flex justify-between items-center py-3 md:px-3 px-2'>
                    ...
                    <ul className={`sm:flex sm:flex-row sm:py-0 sm:px-0 sm:gap-3 sm:bg-transparent sm:items-center text-lg absolute z-[5] sm:static flex-col right-0  ${isActive ? `top-[60px] ` : `top-[-300px]`} duration-500 backdrop-filter backdrop-blur-md py-5 px-4 text-center gap-y-2 bg-[rgba(255,255,255,0.6)]`}>
                        {renderList()}
                    </ul>
                    {
